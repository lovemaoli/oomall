...
public class SelectiveUpdateJpaRepositoryImpl<T, ID> extends SimpleJpaRepository<T, ID> {
    ...
    public <S extends T> S save(S entity) {
        ...
        if (entityInformation.isNew(entity)) {
            logger.debug("save: insert obj");
            em.persist(entity);
        } else {
            logger.debug("save: update obj");
            String sql = null;
            try {
                sql = getSql(entity);
    ...
    private String getSql(Object bo) throws NotFoundException, IllegalAccessException {
        ...
        logger.debug("getSql : sql = {}",stringBuffer.toString());
    ...
    private Object getValueByPropertyName(Object obj, String propertyName) {
        ...
        logger.debug("getValueByPropertyName : {} = {}","method",stringBuffer.toString());
        try {
    ...
    private void setEntityId(Object bo,Object id){
        try {
            bo.getClass().getMethod("setId",id.getClass()).invoke(bo,id);
